 ...
 * measurably reduces buffering on modern WiFi 6/7 chipsets. Below API 29 that mode
 * doesn't exist and ExoPlayer's own high-performance lock already covers it, so this
 * class is a no-op there rather than acquiring a second, redundant high-performance
 * lock. Idempotent: acquiring twice would make the release refcount wrong and leak the
 * lock past the end of playback, which shows up as battery drain rather than as a crash.
 */
    ...
    private val wifiLock: WifiManager.WifiLock? =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            appContext.getSystemService(WifiManager::class.java)
                ?.createWifiLock(WifiManager.WIFI_MODE_FULL_LOW_LATENCY, "$tagPrefix::PlaybackWifiLock")
        } else {
            null
        }
